...
import { UnitfloorPlanProps } from "@/types/unit";

export default function FloorPlan({ data }: { data: UnitfloorPlanProps } ) {
  ...
  return (
    <section className="py-16 px-4 md:px-24 mx-auto">
      {/* Section Header */}
      <div className="text-center mb-12 flex flex-col items-center">
        ...
        <FadeIn delay={0.2}>
          <h2  className="text-center text-2xl md:text-4xl font-bold mb-2 text-Primary">{t.unitGallery.title}</h2>
        </FadeIn>
        <FadeIn delay={0.4} >
      ...
      <div className="flex flex-col space-y-4">
        {data.slice(0, 2).map((item: any, i: number)=> (
          <div key={i} className="w-full px-2 mb-4 cursor-pointer">
            <Image
              key={i}
              src={item.img.url}
