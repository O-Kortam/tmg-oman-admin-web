...
export default function Gallery({ data }: { data: GalleryData }) {
  ...
  return (
    <section className="py-16 px-4 md:px-24 mx-auto bg-gray border-t border-zinc-800/25">
      <div className="text-start mb-8 flex flex-col items-start">
    
       <FadeIn delay={0.2}>
       <h2  className="text-5xl font-semibold text-Primary mb-2">{data?.title}</h2>
       </FadeIn>
         <FadeIn delay={0.2} >
      <p className="text-lg font-medium text-Dark">{data?.desc}</p>
          </FadeIn>
      </div>
